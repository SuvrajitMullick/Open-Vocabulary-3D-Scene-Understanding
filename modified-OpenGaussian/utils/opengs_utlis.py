...
def get_SAM_mask_and_feat(gt_sam_mask, filter_th=50, original_mask_feat=None, sample_mask=False):
    """
    input: 
        gt_sam_mask[1, H, W]: mask id
    output:
    ...
    """
    # (1) mask id: -1, 1, 2, 3,...
    mask_id = gt_sam_mask.clone().squeeze(0) # Convert to [H, W]
    if mask_id.min() < 0:
