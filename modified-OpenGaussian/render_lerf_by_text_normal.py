...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        if not skip_train:
             render_set(dataset.model_path, "text2obj_train", scene.loaded_iter, scene.getTrainCameras(), 
                        gaussians, pipeline, background, scene_name)
        if not skip_test:
             render_set(dataset.model_path, "text2obj_test", scene.loaded_iter, scene.getTestCameras(), 
                        gaussians, pipeline, background, scene_name)
