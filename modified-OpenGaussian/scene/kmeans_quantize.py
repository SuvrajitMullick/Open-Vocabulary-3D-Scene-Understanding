...
class Quantize_kMeans():
    def __init__(self, num_clusters=64, num_leaf_clusters=10, num_iters=10, dim=9, dim_leaf=6):
        ...
        # Track background points (discarded clusters)
    ...
    def equalize_cluster_size(self, mode="root"):
        ...
        # Exclude the dummy background cluster from equalization logic ONLY for root
        if mode == "root":
            num_clusters = self.num_clusters
            valid_mask = unq < num_clusters
            unq = unq[valid_mask]
            n_unq = n_unq[valid_mask]
        elif mode == "leaf":
            num_clusters = self.num_clusters * self.leaf_num_clusters + 1
    ...
    def cluster_assign(self, feat, feat_scaled=None, mode="root", selected_leaf=-1):
        ...
        # Initialize background mask if it doesn't exist
        if self.is_background is None:
            self.is_background = torch.zeros(feat.shape[0]).to(torch.bool).cuda()
            ...
            if chunk:
                ...
                if mode == "root":
                    while True:
                        ...
                        # Force background points to a dummy index so they don't affect centers
                        bg_mask_chunk = self.is_background[i*chunk:(i+1)*chunk]
                        curr_nn_index[bg_mask_chunk] = self.num_clusters

                        # Make one_hot larger to accommodate the dummy index
                        dist = F.one_hot(curr_nn_index, self.num_clusters + 1).type(torch.float32)  
                        # Drop the dummy index for center aggregation
                        dist_active = dist[:, :self.num_clusters]
                        ...
                        curr_centers = self.update_centers_(feat[i*chunk:(i+1)*chunk, :], dist_active, curr_nn_index, avg=False)   # [512, 45]
                        counts += dist_active.detach().sum(0) + 1e-6   # [512]
        ...
        if chunk:
            ...
            if mode == "root":
                while True:
                    ...
                    # Keep background points clamped to dummy index on final pass
                    bg_mask_chunk = self.is_background[i*chunk:(i+1)*chunk]
                    curr_nn_index[bg_mask_chunk] = self.num_clusters
    ...
    def forward(self, gaussian, iteration, assign=False, mode="root", selected_leaf=-1, pos_weight=1.0):
        ...
        if mode == "root":
            centers = self.centers
            vec_dim = self.vec_dim
            # Pad centers with a dummy tensor for root mode's background index
            centers = torch.cat([centers, torch.zeros(1, vec_dim).cuda()], dim=0)
        elif mode == "leaf":
            centers = self.leaf_centers
            vec_dim = self.leaf_vec_dim
        ...
        ins_feat_q = gaussian._ins_feat - gaussian._ins_feat.detach() + sampled_centers[:,:6]
        
        # Revert quantization updates for background points ONLY in root mode
        if mode == "root" and self.is_background is not None:
            ins_feat_q = torch.where(self.is_background.unsqueeze(-1), gaussian._ins_feat, ins_feat_q)
            
        gaussian._ins_feat_q = ins_feat_q

    def replace_with_centers(self, gaussian):
        deg = gaussian._features_rest.shape[1]
        
        # Note: replace_with_centers implicitly targets the root level (uses self.centers/vec_dim)
        centers = torch.cat([self.centers, torch.zeros(1, self.vec_dim).cuda()], dim=0)
        sampled_centers = torch.gather(centers, 0, self.nn_index.unsqueeze(-1).repeat(1, self.vec_dim))
        
        features_rest_q = gaussian._features_rest - gaussian._features_rest.detach() + sampled_centers.reshape(-1, deg, 3)
        
        if self.is_background is not None:
            features_rest_q = torch.where(self.is_background.view(-1, 1, 1), gaussian._features_rest, features_rest_q)
            
        gaussian._features_rest = features_rest_q
