...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, features_folder):
    cam_infos = []
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        mask_seg_path = os.path.join(features_folder, extr.name.split('/')[-1][:-4] + "_s.npy")
        mask_feat_path = os.path.join(features_folder, extr.name.split('/')[-1][:-4] + "_f.npy")
...
def readColmapSceneInfo(path, images, features, eval, llffhold=0):
    try:
    ...
    feature_dir = "language_features" if features == None else features
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics,
                                           images_folder=os.path.join(path, reading_dir), 
                                           features_folder=os.path.join(path, feature_dir))
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)

    if eval:
        with open(os.path.join(path, "sparse/0", "test.txt"), 'r') as file:
            test_cam_names = [line.strip().split(".")[0] for line in file]
        print("test_cam_names:", test_cam_names)
        train_cam_infos = [c for c in cam_infos if c.image_name not in test_cam_names]
...
def readCamerasFromTransforms(path, features, transformsfile, white_background, extension=".png"):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames), desc="load images"):
            ...
            # NOTE: load SAM mask and CLIP feat. [OpenGaussian]
            mask_seg_path = os.path.join(path, features, frame["file_path"].split('/')[-1] + "_s.npy")
            mask_feat_path = os.path.join(path, features, frame["file_path"].split('/')[-1] + "_f.npy")
            if os.path.exists(mask_seg_path):
                sam_mask = np.load(mask_seg_path)    # [level=4, H, W]
            else:
                sam_mask = None
