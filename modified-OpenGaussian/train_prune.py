...
class ClusterObjectnessEvaluator:
    def __init__(self, clip_model, min_pixels=100):
        """
        Args:
            clip_model: OpenCLIPNetwork instance
            min_pixels: minimum visible pixels required to evaluate a cluster
        """
        ...
        # ])
        self.clip.set_negatives([
            "a flat wooden table, desk surface, cutting board, or tablecloth", 
            "a flat room wall, floor, or background architecture",
            "unstructured background room clutter, books, monitors, or electronics",
            "a blurry, out-of-focus background or depth of field effect",
            "a dark cast shadow on a surface",
            "a bright glare, reflection, or lighting artifact",
            "empty background space or uniform flat texture",
            "a semi-transparent cloudy rendering artifact or floater",
            "a smeared, stretched, or distorted pixel texture",
            "a jagged, spiky, or noisy geometric glitch",
            "an amorphous, unrecognizable blob of color",
            "hazy, fog-like noise floating in mid-air",
            "a fragmented digital graphical glitch"
        ])

    def keep_largest_component(self, mask_tensor, area_threshold=0.20):
        """
        Finds the largest connected component in a 2D boolean mask.
        If the largest component makes up less than `area_threshold` (e.g., 20%) 
        of the total active mask area, it discards the mask entirely.
        """
        # Calculate the total number of active pixels in the original mask
        total_mask_area = mask_tensor.sum().item()

        # If the mask is already completely empty, return it as-is
        ...
        # If there's only 1 component, it constitutes 100% of the area, so we keep it
        if num_features <= 1:
            return mask_tensor

        # Count pixels in each component (sizes[0] is the background, which we ignore)
        sizes = np.bincount(labeled_mask.ravel())

        # Find the label and area of the largest component
        largest_label = sizes[1:].argmax() + 1
        largest_component_area = sizes[largest_label]

        # Discard check: If the largest piece is too small relative to the whole
        if largest_component_area < (area_threshold * total_mask_area):
            # Return an empty mask (all False) to kill this cluster for this view
        ...
        return torch.from_numpy(largest_mask).to(mask_tensor.device)
    
    def get_robust_crop_bbox(self, mask, discard_ratio=0.05, margin=15):
        """
        Finds the dense core of a 2D mask by calculating the percentiles of 
        the pixel coordinates, effectively ignoring distant stray pixels.
        
        Args:
            mask: 2D boolean or float tensor [H, W] indicating cluster pixels.
            discard_ratio: The fraction of spatial outliers to ignore on each edge (e.g., 0.05 = 5%).
            margin: Padding to add around the dense core to ensure we don't clip the object.
        """
        # Find the (y, x) coordinates of all pixels belonging to the cluster
        coords = torch.nonzero(mask) # Shape: [num_pixels, 2]
        if coords.shape[0] == 0:
            return None
        
        # Separate Y and X coordinates, convert to float for quantile calculation
        y_coords = coords[:, 0].float()
        x_coords = coords[:, 1].float()

        # Find the dense core boundaries by dropping the extreme spatial outliers
        y_min = int(torch.quantile(y_coords, discard_ratio).item())
        y_max = int(torch.quantile(y_coords, 1.0 - discard_ratio).item())
        x_min = int(torch.quantile(x_coords, discard_ratio).item())
        x_max = int(torch.quantile(x_coords, 1.0 - discard_ratio).item())

        # Add a safety margin and clamp to the actual image dimensions
        H, W = mask.shape
    ...
    def robust_dense_average(self, scores_list, window_width=0.2, min_samples=3, cluster_id=None, high_score_bias=1.0):
        """
        Finds a region of scores that balances high point density with high objectness scores,
        and returns the average of the points in that winning window.
        """
        if len(scores_list) == 0:
            return 0.0

        # If we have too few views, just average them
        if len(scores_list) < min_samples:
            return sum(scores_list) / len(scores_list)

        # Convert to tensor and sort
        tensor_scores = torch.tensor(scores_list, dtype=torch.float32)
        sorted_scores, _ = torch.sort(tensor_scores)
        
        max_window_score = -1.0
        best_window = []

        # Slide a window of 'window_width' across the sorted scores
        for i in range(len(sorted_scores)):
            start_val = sorted_scores[i]
            end_val = start_val + window_width
            
            # Grab all points that fall within this window
            in_window = sorted_scores[(sorted_scores >= start_val) & (sorted_scores <= end_val)]
            current_count = len(in_window)
            
            if current_count > 0:
                # Calculate the weighted score for this window
                mean_score = in_window.mean().item()
                window_score = current_count * (mean_score ** high_score_bias)
                
                # If this window has a higher weighted score, it's the new target
                if window_score > max_window_score:
                    max_window_score = window_score
                    best_window = in_window
                # TIE-BREAKER: If two regions score identically, pick the one with the higher absolute mean
                elif window_score == max_window_score:
                    if mean_score > best_window.mean().item():
                        best_window = in_window

        # Fallback edge case
        if len(best_window) == 0:
            return tensor_scores.mean().item()

        # Average only the optimal core
        return best_window.mean().item()
    
    @torch.no_grad()
    def evaluate(self, scene : Scene, gaussians : GaussianModel, renderFunc, pipe, background, cluster_ids, num_clusters, iteration):
        
        cluster_scores_dict = collections.defaultdict(list)
        cameras = scene.getTrainCameras()
        total_views = len(cameras)
        cluster_view_counts = torch.zeros(num_clusters, device="cuda")
        
        for view in cameras:
            if not view.data_on_gpu:
                view.to_gpu()
                
            # Render cluster-level outputs 
            render_pkg = renderFunc(view, gaussians, pipe, background, 0,
                                    cluster_idx=cluster_ids,
                                    render_feat_map=False,
                                    render_cluster=True,
                                    seg_rgb=True,
                                    better_vis=True,
                                    root_num=num_clusters, leaf_num=10)

            cluster_occur = render_pkg["cluster_occur"]
            cluster_imgs = render_pkg["cluster_imgs"]
            cluster_masks = render_pkg["cluster_silhouettes"]
            ...
            occured_cluster_ids = torch.where(cluster_occur)[0]
            for i, real_cid in enumerate(occured_cluster_ids):
                real_cid_int = int(real_cid.item())
                if real_cid_int < 0 or real_cid_int >= num_clusters:
                    continue
                
                img = cluster_imgs[i]
                mask = cluster_masks[i] > 0.7
                mask = self.keep_largest_component(mask)
                if mask.sum() < self.min_pixels:
                    continue
                
                cluster_view_counts[real_cid_int] += 1
                bbox = self.get_robust_crop_bbox(mask, discard_ratio=0.05, margin=15)
                if bbox is None:
                    continue
                y_min, y_max, x_min, x_max = bbox
                
                img_crop = img[:3, y_min:y_max, x_min:x_max]
                mask_crop = mask[y_min:y_max, x_min:x_max].unsqueeze(0)
                masked_crop = img_crop * mask_crop

                clip_input = F.interpolate(masked_crop.unsqueeze(0), size=(224, 224), mode='bilinear')
                embed = self.clip.encode_image(clip_input)
                ...
                cluster_scores_dict[real_cid_int].append(score_val)
                
                # # ==========================================
                # # SAVE CLUSTER IMAGES & MASKS WITH SCORES
                # # ==========================================
                # cluster_dir = os.path.join(scene.model_path, "eval_clusters", f"iter_{iteration}", f"cluster_{real_cid_int:04d}")
                # rgb_dir = os.path.join(cluster_dir, "rgb")
                # mask_dir = os.path.join(cluster_dir, "mask")
                
                # makedirs(rgb_dir, exist_ok=True)
                # makedirs(mask_dir, exist_ok=True)
                
                # # Convert the masked tensor back into a PIL image
                # img_np = (masked_crop.cpu().permute(1, 2, 0).numpy() * 255).astype(np.uint8)
                # pil_img = Image.fromarray(img_np)
                # draw = ImageDraw.Draw(pil_img)
                
                # text_str = f"{score_val:.2f}"
                
                # # Dynamically calculate text size (handles different PIL versions)
                # try:
                #     left, top, right, bottom = draw.textbbox((0, 0), text_str)
                #     tw, th = right - left, bottom - top
                # except AttributeError:
                #     tw, th = draw.textsize(text_str)
                    
                # # Center the text
                # img_w, img_h = pil_img.size
                # x, y = (img_w - tw) / 2, (img_h - th) / 2
                
                # # Draw the white background rectangle slightly larger than the text
                # draw.rectangle([x - 2, y - 2, x + tw + 2, y + th + 2], fill="white")
                
                # # Draw the black text over it
                # draw.text((x, y), text_str, fill="black")
                
                # # Save RGB (Masked) with Score
                # pil_img.save(os.path.join(rgb_dir, f"{view.image_name}.png"))
                
                # # Save Mask
                # torchvision.utils.save_image(mask_crop.to(torch.float32), os.path.join(mask_dir, f"{view.image_name}.png"))
                # # ==========================================
        ...
        # POST-PROCESS: ROBUST AGGREGATION
        final_scores = torch.zeros(num_clusters, device="cuda")
        
        for cid in range(num_clusters):
            scores_list = cluster_scores_dict.get(cid, [])
            if len(scores_list) == 0:
                continue
            final_scores[cid] = self.robust_dense_average(scores_list, cluster_id=cid,  window_width=0.15, high_score_bias=0.5)
        visibility_ratios = cluster_view_counts / total_views
        
        # # ==========================================
        # # PRINT FINAL SCORES PER CLUSTER
        # # ==========================================
        # final_scores_list = final_scores.cpu().tolist()
        
        # # Format the numbers to 3 decimal places with their Cluster ID (e.g., "C000: 0.450")
        # formatted_scores = [f"C{i:03d}: {score:.3f}" for i, score in enumerate(final_scores_list)]
        
        # str_final_scores = " | ".join(formatted_scores)
        
        # print(f"\n--- [ITERATION {iteration}] FINAL SCORES PER CLUSTER ---")
        # print(f"{str_final_scores}\n")
        # # ==========================================

        return final_scores, visibility_ratios
...
def save_kmeans(kmeans_list, quantized_params, out_dir, mode="root"):
    ...
    for kmeans in kmeans_list:
        ...
        bitarr = bitarray(assignments.cpu().numpy().flatten().tolist())
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, \
    ...
    # Initialize Evaluator for dynamic adjustment
    objectness_threshold = 0.2
    clip_model = OpenCLIPNetwork(OpenCLIPNetworkConfig())
    objectness_evaluator = ClusterObjectnessEvaluator(clip_model)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration > opt.start_root_cb_iter and iteration <= opt.start_leaf_cb_iter:
            ...
            # ###########################################################################
            # [Stage 2.1] Targeted Split-and-Merge Cluster Adjustment                   #
            # ###########################################################################
            if (iteration - opt.start_root_cb_iter) > 1 and (iteration - opt.start_root_cb_iter) % 1000 == 1:
                old_root_num = opt.root_node_num
                cluster_scores, visibility_ratios = objectness_evaluator.evaluate(scene, gaussians, render, pipe, background, ins_feat_codebook.cls_ids, old_root_num, iteration)
                
                remove_mask = (cluster_scores < objectness_threshold) & (visibility_ratios < 0.25)
                keep_mask = ~remove_mask
                
                num_removed = remove_mask.sum().item()
                removed_ids = torch.where(remove_mask)[0]
                removed_ids_list = removed_ids.tolist()
                
                for r_id in removed_ids:
                    ins_feat_codebook.is_background[ins_feat_codebook.cls_ids == r_id] = True
                
                kept_centers = ins_feat_codebook.centers[keep_mask]
                ins_feat_codebook.centers = kept_centers
                new_root_num = ins_feat_codebook.centers.shape[0]
                
                opt.root_node_num = new_root_num
                ins_feat_codebook.num_clusters = new_root_num
                print(f"\n[Stage 2.1] Iteration {iteration}: PRUNED {num_removed} clusters below score {objectness_threshold}. Removed IDs: {removed_ids_list}. Total Clusters: {new_root_num}.")
        ...
        if viewpoint_cam.original_sam_mask is not None:
            gt_sam_mask = viewpoint_cam.original_sam_mask.cuda()    # [1, H, W]
        ...
        if iteration > opt.start_ins_feat_iter:
            ...
            mask_id, mask_bool, invalid_pix = get_SAM_mask_and_feat(gt_sam_mask, filter_th=50)
        ...
        if (iteration % save_fre == 0) and save_intermediate:
            ...
            if iteration > opt.start_ins_feat_iter:
                ...
                if gt_sam_mask is not None and iteration > opt.start_ins_feat_iter:
                    ...
                    gt_sam_path = os.path.join(scene.model_path, "train_process", sub_floader, "gt_sam_mask")
...
def construct_pseudo_ins_feat(scene : Scene, renderFunc, renderArgs, 
    ...
    for idx, view in enumerate(tqdm(sorted_train_cameras, desc="construt pseudo feat")):
        ...
        mask_id, mask_bool, invalid_pix = \
            get_SAM_mask_and_feat(view.original_sam_mask.cuda())
        ...
        # raw mask [1+num_mask, H, W]
        mask_bool_raw = torch.cat((torch.zeros_like(mask_bool[0].unsqueeze(0)), mask_bool), dim=0)
        ...
        view.pesudo_mask_bool = mask_bool_filtered.to(torch.bool) if filter else mask_bool_raw.to(torch.bool)
    ...
    if mode == "lang":
        ...
        for v_id, view in enumerate(sorted_train_cameras):
            if not view.data_on_gpu:
                view.to_gpu()
            curr_view_lang_feat = view.original_mask_feat   # [num_mask, 512]
