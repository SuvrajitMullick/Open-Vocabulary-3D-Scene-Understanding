...
class ContextAwareSemanticHierarchicalUnifier:
    ...
    def unify(self, masks_l, masks_m, masks_s, image):
        ...
        for i, parent_l in enumerate(masks_l):
            ...
            parent_l['avg_visual_sim_m'] = avg_visual_sim_m
            parent_l['dyn_split_thresh_m'] = dyn_split_thresh_m
...
def visualize_masks(image_rgb, unified_masks, masks_l, save_path):
    h, w = image_rgb.shape[:2]
    vis_image = np.zeros((h, w, 3), dtype=np.uint8)
    
    # Pass 1: Draw all final unified masks
    np.random.seed(42)
    color_palette = np.random.randint(50, 256, size=(1000, 3), dtype=np.uint8)
    for i, m in enumerate(unified_masks):
        color = color_palette[i % len(color_palette)].tolist()
        vis_image[m['segmentation']] = color

    # Pass 2: Identify evaluated parents and format text lines
    text_annotations = []
    for m in masks_l:
        if 'avg_visual_sim_m' in m and 'dyn_split_thresh_m' in m:
            lines = [
                f"Obj: {m['objectness']:.2f}",
                f"Cpx: {m['complexity']:.2f}",
                f"Sim: {m['avg_visual_sim_m']:.2f}",
                f"Thr: {m['dyn_split_thresh_m']:.2f}"
            ]
            x, y, w_box, h_box = map(int, m['bbox'])
            cx = x + w_box / 2
            cy = y + h_box / 2
            text_annotations.append((lines, cx, cy))

    # Pass 3: Draw multiline text with a shared white background
    ...
    for lines, cx, cy in text_annotations:
        # Pre-calculate sizes for all lines in the block
        ...
        total_height = sum([size[0][1] for size in text_sizes]) + (len(lines) - 1) * line_spacing
        
        # Calculate top-left coordinate to center the whole block at (cx, cy)
        box_x = int(cx - max_width / 2)
        box_y = int(cy - total_height / 2)
        
        # Calculate bounding box coordinates
        top_left = (box_x - pad, box_y - pad)
        # Adding the baseline of the final line to the bottom_right y-coordinate
        bottom_right = (box_x + max_width + pad, box_y + total_height + text_sizes[-1][1] + pad)
        ...
        cv2.rectangle(vis_image, top_left, bottom_right, (255, 255, 255), -1)
        cv2.rectangle(vis_image, top_left, bottom_right, (0, 0, 0), 1)
        
        # Draw each line of text
        curr_y = box_y
        for i, line in enumerate(lines):
            line_height = text_sizes[i][0][1]
            curr_y += line_height
            cv2.putText(vis_image, line, (box_x, curr_y), font, font_scale, (0, 0, 0), thickness)
            curr_y += line_spacing # Move down for the next line
...
def create(image_list, data_list, save_folder):
    ...
    for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False):
        timer += 1
        try:
            img_embed, seg_map = _embed_clip_sam_tiles(img.unsqueeze(0), sam_encoder, data_list[i], vis_folder)
        except:
...
def _embed_clip_sam_tiles(image, sam_encoder_fn, image_name, vis_folder):
    aug_imgs = torch.cat([image])
    seg_images, seg_map = sam_encoder_fn(aug_imgs, image_name, vis_folder)
...
def sam_encoder(image, image_name, vis_folder):
    ...
    unified_masks = unifier.unify(masks_l, masks_m, masks_s, image)
    
    vis_path = os.path.join(vis_folder, image_name.split('.')[0] + '_sim_thresh.png')
    visualize_masks(image, unified_masks, masks_l, vis_path)
    ...
    seg_images, seg_maps = {}, {}
    seg_images['unified'], seg_maps['unified'] = mask2segmap(unified_masks, image)
