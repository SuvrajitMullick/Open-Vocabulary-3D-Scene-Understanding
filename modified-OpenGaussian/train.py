...
class ClusterObjectnessEvaluator:
    ...
    def robust_dense_average(self, scores_list, window_width=0.2, min_samples=3, high_score_bias=1.0):
        """
    ...
    def evaluate(self, scene : Scene, gaussians : GaussianModel, renderFunc, pipe, background, cluster_ids, num_clusters, iteration):
        ...
        for cid in range(num_clusters):
            ...
            if len(scores_list) == 0:
                continue
            final_scores[cid] = self.robust_dense_average(scores_list, window_width=0.15, high_score_bias=0.5)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if iteration > opt.start_root_cb_iter and iteration <= opt.start_leaf_cb_iter:
            ...
            if (iteration - opt.start_root_cb_iter) > 1 and (iteration - opt.start_root_cb_iter) % 1000 == 1:
                ...
                cluster_scores = objectness_evaluator.evaluate(scene, gaussians, render, pipe, background, ins_feat_codebook.cls_ids, old_root_num, iteration)
                
                keep_mask = cluster_scores >= objectness_threshold
                remove_mask = cluster_scores < objectness_threshold
