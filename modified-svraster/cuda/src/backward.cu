...
renderCUDA(
    ...
    const float* __restrict__ dL_dout_object,
    ...
    float* dL_dvox)
{
    ...
    __shared__ float16 collected_obj[BLOCK_SIZE];
    ...
    if (inside)
    {
        ...
        for (int k = 0; k < 16; ++k)
            dL_dpix_obj.d[k] = dL_dout_object[k * H * W + pix_id];
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        if (range.x + progress < range.y)
        {
            ...
            collected_obj[thread_id] = objs[vox_id];
        }
        block.sync();
        ...
        for (int jj = 0; !done && jj <= j_lst_top; jj++)
        {
            ...
            const float16 o = collected_obj[j];

            // The gradients w.r.t. voxel alpha.
            float dL_dpt_w = dot(dL_dpix, c);
            #pragma unroll
            for (int k = 0; k < 16; ++k)
                dL_dpt_w += dL_dpix_obj.d[k] * o.d[k];
            ...
            **************************/
            ...
            #pragma unroll
            for (int k = 0; k < 16; ++k)
                dL_dobj[k] = pt_w * dL_dpix_obj.d[k];
            ...
            **************************/
            float grad_pack[28];  // 8 geo + 3 rgb + 16 obj + 1 alpha
            #pragma unroll
            for (int iii=0; iii<8; ++iii)
            ...
            #pragma unroll
            for (int k = 0; k < 16; ++k)
                grad_pack[11 + k] = dL_dobj[k];
            grad_pack[27] = fabs(dL_dalpha * alpha);

            const int base_id = cg::this_grid().thread_rank();
            #pragma unroll
            for (int iii=0; iii<28; ++iii)
                atomicAdd(dL_dvox + vox_id * 28 + (base_id + iii) % 28, grad_pack[(base_id + iii) % 28]);
        }
    }
}
...
rasterize_voxels_backward(
    ...
    const torch::Tensor& dL_dout_object,
    ...
    const bool debug)
{
    ...
    if (P == 0)
    {
        ...
        torch::Tensor dL_dobjs = torch::empty({0});
        torch::Tensor subdiv_p_bw = torch::empty({0});
        return std::make_tuple(dL_dgeos, dL_drgbs, dL_dobjs, subdiv_p_bw);
    }

    torch::Tensor dL_dvox = torch::zeros({P, geos.size(1)+3+16+1}, vox_centers.options());
    ...
    // Compute loss gradients w.r.t. surface property, voxel color and voxel object.
    render(
        tile_grid, block,
        imgState.ranges,
        ...
        dL_dout_object.contiguous().data_ptr<float>(),
    ...
    std::vector<torch::Tensor> gradient_lst = dL_dvox.split({geos.size(1), 3, 16, 1}, 1);
    torch::Tensor dL_dgeos = gradient_lst[0].contiguous();
    torch::Tensor dL_drgbs = gradient_lst[1].contiguous();
    torch::Tensor dL_dobjs = gradient_lst[2].contiguous();
    torch::Tensor subdiv_p_bw = gradient_lst[3].contiguous();
