    ...
    float* __restrict__ max_w)
{
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        for (int jj = 0; !done && jj <= j_lst_top; jj++)
        {
            ...
            #pragma unroll
            for (int k = 0; k < 16; ++k)
                O.d[k] += pt_w * collected_obj[j].d[k];
    ...
    const bool debug)
{
    ...
    if (objs.ndimension() != 2 || objs.size(1) != 16)
        AT_ERROR("objs should be either (num_points, 16)");
    if (vox_centers.size(0) != rgbs.size(0))
        AT_ERROR("size mismatch");
    if (vox_centers.size(0) != objs.size(0))
        AT_ERROR("size mismatch");
    ...
    torch::Tensor out_object = torch::full({16, H, W}, 0.f, float_opts);
    ...
    if(P != 0)
        rendered = rasterize_voxels_procedure(
            ...
            objs.contiguous().data_ptr<float>(),

            out_color.contiguous().data_ptr<float>(),
            out_object.contiguous().data_ptr<float>(),
    ...
    return std::make_tuple(rendered, binningBuffer, imgBuffer, out_color, out_object, out_depth, out_normal, out_T, max_w);
}
