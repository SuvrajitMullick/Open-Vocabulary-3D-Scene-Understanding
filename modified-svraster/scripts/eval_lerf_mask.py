...
def evaluate(out_dir, base_mode, split, iteration):
    gt_path = os.path.join('data/label', os.path.basename(out_dir))
    pred_path = os.path.join(out_dir, 'text2obj_' + split, 'ours_' + iteration, 'objects_render')
    ...
    for image_name in os.listdir(gt_path):
        ...
        pred_image_path = os.path.join(pred_path, image_name, 'masks')
    ...
    results_path = os.path.join(out_dir, "results_segmentation.json")
    with open(results_path, 'w') as fp:
        json.dump(full_dict, fp, indent=4)
    
    per_view_path = os.path.join(out_dir, "per_view_segmentation.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--iteration", type=str, default="20000")
