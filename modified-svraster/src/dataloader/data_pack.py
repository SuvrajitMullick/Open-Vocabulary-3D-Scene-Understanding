...
class CameraCreator:
    ...
    def __call__(self,
        ...
        if tensor.shape[0] == 4:
            # Blend alpha channel
            tensor, alpha_mask = tensor.split([3, 1], dim=0)
            if not self.skip_blend_alpha:
                tensor = tensor * alpha_mask + int(self.alpha_is_white) * (1 - alpha_mask)
        ...
        # Convert object to tensor if there is
        if object is not None:
            size = tensor.shape[-2:][::-1]
            if object.size != size:
                object = object.resize(size)
            object = torch.tensor(np.array(object), dtype=torch.float32)
            if len(object.shape) == 3:
                object = object.mean(-1)
