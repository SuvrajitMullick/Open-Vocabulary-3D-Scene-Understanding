...
class SVRenderer:
    ...
    def vox_fn(self, idx, cam_pos, color_mode=None, object_mode=None, viewdir=None):
        '''
        ...
        3. Compute object for each voxel.
        ...
        # Compute voxel objects
        if object_mode is None or object_mode == "sh":
            objs = svraster_cuda.renderer.SH_OBJ_eval.apply(
                idx,
                self.vox_center,
                viewdir, # Ignore above one when viewdir is not None
                self.sh_objs,
            )
        elif object_mode == "rand":
            objs = torch.rand([self.num_voxels, 16], dtype=torch.float32, device="cuda")
        elif object_mode == "dontcare":
            objs = torch.empty([self.num_voxels, 16], dtype=torch.float32, device="cuda")
        else:
            raise NotImplementedError
    ...
    def render(
        ...
        raster_settings = svraster_cuda.renderer.RasterSettings(
            ...
            **other_opt)
        color, object, depth, normal, T, max_w = svraster_cuda.renderer.rasterize_voxels(
            raster_settings,
            self.octpath,
            self.vox_center,
            self.vox_size,
        ...
        for k in ['color', 'object', 'depth', 'normal', 'T']:
    ...
    def camera2rasterizer(self, camera):
        ss = self.ss
        w_src, h_src = camera.image_width, camera.image_height
        w, h = round(w_src * ss), round(h_src * ss)
        w_ss, h_ss = w / w_src, h / h_src
        ...
        raster_settings = svraster_cuda.renderer.RasterSettings(
            color_mode="dontcare",
            object_mode="dontcare",
            n_samp_per_vox=self.n_samp_per_vox,
            image_width=w_src,
            image_height=h_src,
            tanfovx=camera.tanfovx,
            tanfovy=camera.tanfovy,
    ...
    def apply_weights(self, camera, weights, cnt, image_weights):
        assert weights is not None
        assert cnt is not None
        assert image_weights is not None

        raster_settings = self.camera2rasterizer(camera)

        # Preprocess geometry (same as in rasterize_voxels), for ALL voxels
        n_duplicates, geomBuffer = svraster_cuda.renderer.mark_n_duplicates(
            ...
            cx=raster_settings.cx,
            cy=raster_settings.cy,
            w2c_matrix=raster_settings.w2c_matrix,
            c2w_matrix=raster_settings.c2w_matrix,
            near=raster_settings.near,
            octree_paths=self.octpath,
            vox_centers=self.vox_center,
            vox_lengths=self.vox_size,
            return_buffer=True,
            debug=raster_settings.debug,
        )
        in_frusts_idx = torch.where(n_duplicates > 0)[0]
        vox_params = self.vox_fn(in_frusts_idx, None, raster_settings.color_mode, raster_settings.object_mode)
        geos = vox_params['geos']

        # Call the low-level apply-weights rasterizer
        svraster_cuda.renderer.rasterize_voxels_apply_weights(
            raster_settings,
            self.octpath,
            self.vox_center,
            self.vox_size,
