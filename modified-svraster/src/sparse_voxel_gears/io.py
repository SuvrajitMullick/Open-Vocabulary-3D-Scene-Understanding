...
class SVInOut:
    ...
    def save(self, path, quantize=False):
        ...
        state_dict = {
            ...
            '_sh_objs': self._sh_objs.data.contiguous(),
        }
    ...
    def load(self, path):
        ...
        self._sh_objs = state_dict['_sh_objs'].cuda().requires_grad_()
    ...
    def load_iteration(self, model_path, iteration=-1):
        if iteration == -1:
            ...
            loaded_iter = max(int(digits) for fname in fnames if (digits := re.sub("[^0-9]", "", fname)) != "")
        else:
...
def quantize_state_dict(state_dict):
    ...
    state_dict['_sh_objs'] = [quantization(v) for v in state_dict['_sh_objs'].split(1, dim=1)]
...
def dequantize_state_dict(state_dict):
    ...
    state_dict['_sh_objs'] = torch.cat(
        [dequantization(v) for v in state_dict['_sh_objs']], dim=1)
