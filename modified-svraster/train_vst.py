...
def training(args):
    ...
    def create_trainer():
        ...
        optimizer = svraster_cuda.sparse_adam.SparseAdam(
            [
                ...
                {'params': [voxel_model._sh_objs], 'lr': cfg.optimizer.sh_objs_lr},
            ],
    ...
    for iteration in iter_rng:
        ...
        render_object = render_pkg['object']
        ...
        # # Hypersphere Regularization for 3D Gaussian Objects
        # loss_3d_hypersphere = None
        # if iteration % cfg.optimizer.hyp3d_interval == 0:
        #     feat_3d = voxel_model.sh_objs
        #     feat_3d_norm = torch.norm(feat_3d, dim=-1) # L2 norm per Gaussian
        #     loss_3d_hypersphere = ((feat_3d_norm - 1.0) ** 2).mean() # (||f|| - 1)^2 averaged over all Gaussians
        #     loss += cfg.regularizer.hypersphere_lambda_3d * loss_3d_hypersphere

        # # Rendered Feature Regularization for 2D Gaussian Splatted Objects
        # feat_2d = render_object.view(voxel_model.num_objects, -1).permute(1, 0) # [C, H, W] -> [H*W, C]
        # feat_2d_norm = torch.norm(feat_2d, dim=-1) # L2 norm per pixel feature
        # loss_2d_hypersphere = ((feat_2d_norm - 1.0) ** 2).mean() # (||F_p||_2 - 1)^2 averaged over all pixels
        # loss += cfg.regularizer.hypersphere_lambda_2d * loss_2d_hypersphere
        
        # Object Loss 2D
        gt_object = cam.object.cuda().long()
        logits_2D = classifier(render_object)
        loss_obj_2D = cls_criterion(logits_2D.unsqueeze(0), gt_object.unsqueeze(0)).squeeze().mean()
        loss_obj_2D = loss_obj_2D / torch.log(torch.tensor(num_classes))  # normalize to (0,1)
        loss += cfg.regularizer.lambda_object_2D * loss_obj_2D
        # print("loss_obj_2D:", loss_obj_2D)
        
        # Object Loss 3D
        if iteration % cfg.optimizer.reg3d_interval == 0:
            # regularize at certain intervals
            logits_3D = classifier(voxel_model.sh_objs.unsqueeze(0).permute(2,1,0))
            prob_obj_3D = torch.softmax(logits_3D,dim=0).squeeze().permute(1,0)
            loss_obj_3D = loss_utils.loss_cls_3D(voxel_model.vox_center.detach(), prob_obj_3D, cfg.optimizer.reg3d_k, cfg.optimizer.reg3d_max_points, cfg.optimizer.reg3d_sample_size)
            loss_obj_3D = loss_obj_3D / num_classes  # normalize to (0,1)
            loss += cfg.regularizer.lambda_object_3D * loss_obj_3D
            # print("loss_obj_3D:", loss_obj_3D)
             
        # # Contastive Loss 2d
        # contrast_loss_2d = loss_utils.contrastive_loss_2d(render_object, gt_object)
        # loss += cfg.regularizer.contrastive_lambda_2d * contrast_loss_2d
        
        # # Contastive Loss 3d
        # contrast_loss_3d = None
        # if iteration % cfg.optimizer.con3d_interval == 0:
        #     contrast_loss_3d = loss_utils.contrastive_loss_3d(voxel_model.vox_center.detach(), voxel_model.sh_objs)
        #     loss += cfg.regularizer.contrastive_lambda_3d * contrast_loss_3d
        
        # Weight Object Losses
        loss_obj_wt_3d = None
        # regularize at certain intervals
        if iteration % cfg.optimizer.reg3d_wt_interval == 0:
            colmap_cameras = data_pack.get_train_cameras().copy()
            N_voxel = voxel_model.sh_objs.shape[0]

            weights = torch.zeros(N_voxel, num_classes, device="cuda") # [Num_of_Voxels, Num_of_Classes]
            weights_cnt = torch.zeros_like(weights, dtype=torch.float32) # [Num_of_Voxels, Num_of_Classes]

            total_view_num = len(colmap_cameras)
            view_indices = random.sample(range(total_view_num), min(total_view_num, cfg.regularizer.num_of_views))
        
            for idx in view_indices:
                cam = colmap_cameras[idx]
                label = cam.object.cuda().long()  # (H,W)
                unique_classes = label.unique().tolist()
                
                for cls in unique_classes:
                    mask = (label == cls).float().unsqueeze(0)  # (1,H,W)
                    w_cls = torch.zeros(N_voxel, device="cuda", dtype=torch.float32)
                    cnt_cls = torch.zeros(N_voxel, device="cuda", dtype=torch.float32)

                    voxel_model.apply_weights(cam, w_cls, cnt_cls, mask)
                    weights[:, cls] += w_cls
                    weights_cnt[:, cls] += cnt_cls

            weights = weights / (weights_cnt.clamp_min(1).float())
            weights_prob = weights / (weights.sum(dim=-1, keepdim=True) + 1e-9)  # [Num_of_Voxels, Num_of_Classes]
            weights_prob = weights_prob.detach()
            total_hits = weights_cnt.sum(dim=-1)
            valid_mask = (total_hits > 0).float() # Shape: [N_voxel]

            start_T = 1.0
            end_T = 0.1
            progress = min(1.0, iteration / cfg.procedure.n_iter) 
            current_T = start_T * (end_T / start_T) ** progress
            
            sharpened_weights = (weights_prob + 1e-9) ** (1.0 / current_T)
            sharpened_weights = sharpened_weights / (sharpened_weights.sum(dim=-1, keepdim=True) + 1e-12)
            logits_3d = classifier(voxel_model.sh_objs.unsqueeze(0).permute(2,1,0))
            log_prob_obj_3d = F.log_softmax(logits_3d, dim=0).squeeze().permute(1,0)

            kl_unreduced = F.kl_div(log_prob_obj_3d, sharpened_weights, reduction="none")
            class_frequencies = sharpened_weights.sum(dim=0) # Shape: [num_classes]
            class_weights = 1.0 / (class_frequencies ** 0.5 + 1e-6)
            class_weights = class_weights / class_weights.mean()
            kl_weighted = kl_unreduced * class_weights.unsqueeze(0)

            loss_per_voxel = kl_weighted.sum(dim=-1) # Shape: [N_voxel]
            loss_obj_wt_3d = (loss_per_voxel * valid_mask).sum() / (valid_mask.sum() + 1e-6)
            loss_obj_wt_3d = loss_obj_wt_3d / num_classes  # normalize to (0,1)
            loss += cfg.regularizer.lambda_object_3D_wt * loss_obj_wt_3d
            # print("loss_obj_wt_3d:", loss_obj_wt_3d)
        ...
        with torch.no_grad():
            ...
            if iteration in args.checkpoint_iterations or iteration == cfg.procedure.n_iter:
                ...
                torch.save(classifier.state_dict(), os.path.join(args.model_path, "checkpoints", 'classifier.pth'))
...
if __name__ == "__main__":
    ...
    if cfg.procedure.sche_mult != 1:
        ...
        for key in ['geo_lr', 'sh0_lr', 'shs_lr', 'sh_objs_lr']:
