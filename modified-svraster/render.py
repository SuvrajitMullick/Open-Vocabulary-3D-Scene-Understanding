...
@torch.no_grad()
def render_set(name, iteration, suffix, args, views, voxel_model, classifier):

    im_render_path = os.path.join(args.model_path, name, f"ours_{iteration}{suffix}", "renders")
    im_gts_path = os.path.join(args.model_path, name, f"ours_{iteration}{suffix}", "gt")
    ...
    obj_feature_path = os.path.join(args.model_path, name, f"ours_{iteration}{suffix}", "objects_feature16")
    obj_gts_path = os.path.join(args.model_path, name, f"ours_{iteration}{suffix}", "gt_objects_color")
    obj_render_path = os.path.join(args.model_path, name, f"ours_{iteration}{suffix}", "objects_pred")
    ...
    makedirs(obj_feature_path, exist_ok=True)
    makedirs(obj_gts_path, exist_ok=True)
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if not args.eval_fps:
            rendering_image = render_pkg['color']
            im_gt = view.image.cuda()
            mse = (rendering_image.clip(0,1) - im_gt.clip(0,1)).square().mean()
            psnr = -10 * torch.log10(mse)
            psnr_lst.append(psnr.item())
            fname = view.image_name
            
            rendering_object = render_pkg['object']
            
            logits = classifier(rendering_object)
            pred_obj = torch.argmax(logits,dim=0)
            pred_obj_mask = visualize_obj(pred_obj.cpu().numpy().astype(np.uint8))
        
            ob_gt = view.object
            gt_rgb_mask = visualize_obj(ob_gt.cpu().numpy().astype(np.uint8))

            rgb_mask = feature_to_rgb(rendering_object)
            Image.fromarray(rgb_mask).save(os.path.join(obj_feature_path, '{0:05d}'.format(idx) + ".png"))
            Image.fromarray(gt_rgb_mask).save(os.path.join(obj_gts_path, '{0:05d}'.format(idx) + ".png"))
            Image.fromarray(pred_obj_mask).save(os.path.join(obj_render_path, '{0:05d}'.format(idx) + ".png"))

            # RGB
            imageio.imwrite(
                os.path.join(im_render_path, fname + (".jpg" if args.use_jpg else ".png")),
                im_tensor2np(rendering_image)
            )
            if args.rgb_only:
                continue
            imageio.imwrite(
                os.path.join(im_gts_path, fname + ".png"),
                im_tensor2np(im_gt)
            )
            # Alpha
            imageio.imwrite(
