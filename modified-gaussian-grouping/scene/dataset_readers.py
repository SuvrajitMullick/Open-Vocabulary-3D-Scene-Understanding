...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, objects_folder, depths_folder, test_cam_names_list):
    cam_infos = []
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, depth_params=depth_params, image=image,
                              image_path=image_path, image_name=image_name, depth_path=depth_path, objects=objects,
...
def readColmapSceneInfo(path, images, depths, eval, object_path, train_test_exp, llffhold=0, n_views=100, random_init=False):
    try:
    ...
    if eval:
        ...
        if llffhold:
            ...
            test_cam_names_list = [name.split(".")[0] for idx, name in enumerate(cam_names) if idx % llffhold == 0]
        else:
            with open(os.path.join(path, "sparse/0", "test.txt"), 'r') as file:
                test_cam_names_list = [line.strip().split(".")[0] for line in file]
            print("test_cam_names:", test_cam_names_list)
    else:
        test_cam_names_list = []
