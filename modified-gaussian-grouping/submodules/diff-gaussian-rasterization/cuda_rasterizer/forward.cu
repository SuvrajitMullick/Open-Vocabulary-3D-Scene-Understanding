...
renderCUDA(
	...
	float* __restrict__ sampled_T, float* __restrict__ sampled_ar, float* __restrict__ sampled_aro, float* __restrict__ sampled_ard,
	int W, int H,
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (j % 32 == 0) {
				...
				for (int ch = 0; ch < OBJECTS; ++ch) {
					sampled_aro[(bbm * BLOCK_SIZE * OBJECTS) + ch * BLOCK_SIZE + block.thread_rank()] = O[ch];
				}
...
void FORWARD::render(
	...
	float* sampled_T, float* sampled_ar, float* sampled_aro, float* sampled_ard,
	int W, int H,
	...
	float* depth)
{
	renderCUDA<NUM_CHANNELS_3DGS, NUM_OBJECTS> << <grid, block >> > (
		ranges,
		point_list,
...
void FORWARD::preprocess(int P, int D, int M,
	...
	bool antialiasing)
{
	preprocessCUDA<NUM_CHANNELS_3DGS, NUM_OBJECTS> << <(P + 255) / 256, 256 >> > (
		P, D, M,
		means3D,
		scales,
		scale_modifier,
