...
CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk(char*& chunk, size_t N)
{
	...
	obtain(chunk, img.pixel_objects, N * NUM_OBJECTS, 128);
...
CudaRasterizer::SampleState CudaRasterizer::SampleState::fromChunk(char *& chunk, size_t C) {
	...
	obtain(chunk, sample.aro, NUM_OBJECTS * C * BLOCK_SIZE, 128);
	...
	bool debug)
{
	...
	CHECK_CUDA(FORWARD::render(
		...
		sampleState.T, sampleState.ar, sampleState.aro, sampleState.ard,
		width, height,
		geomState.means2D,
		feature_ptr,
		sh_objs,
	...
	CHECK_CUDA(cudaMemcpy(imgState.pixel_objects, out_objects, sizeof(float) * width * height * NUM_OBJECTS, cudaMemcpyDeviceToDevice), debug);
	...
	bool debug) 
{
	...
	dim3 block(BLOCK_X, BLOCK_Y, 1);
  	...
  	if (NUM_CHANNELS_3DGS != 3 && weights == nullptr)
	{
    	throw std::runtime_error("For non-RGB, provide precomputed Gaussian colors!");
  	}
	...
	CHECK_CUDA(, debug)
  	...
  	// Let each tile blend its range of Gaussians independently in parallel
  	float* feature_ptr = weights != nullptr ? weights : geomState.rgb;
