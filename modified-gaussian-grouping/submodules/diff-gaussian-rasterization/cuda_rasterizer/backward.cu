...
PerGaussianRenderCUDA(
	...
	const float* __restrict__ sampled_T, const float* __restrict__ sampled_ar, const float* __restrict__ sampled_aro, const float* __restrict__ sampled_ard,
	...
	for (int i = 0; i < BLOCK_SIZE + 31; ++i) {
		...
		for (int ch = 0; ch < O; ++ch) {
			aro[ch] = my_warp.shfl_up(aro[ch], 1);
			dL_dpixel_obj[ch] = my_warp.shfl_up(dL_dpixel_obj[ch], 1);
		}
		...
		if (valid_splat && valid_pixel && my_warp.thread_rank() == 0 && idx < BLOCK_SIZE) {
			...
			for (int ch = 0; ch < O; ++ch)
				aro[ch] = -pixel_objects[ch * H * W + pix_id] + sampled_aro[global_bucket_idx * BLOCK_SIZE * O + ch * BLOCK_SIZE + idx];
			...
			for (int ch = 0; ch < O; ++ch) {
				dL_dpixel_obj[ch] = dL_dpixels_objs[ch * H * W + pix_id];
			}
		...
		if (valid_splat && valid_pixel && 0 <= idx && idx < BLOCK_SIZE) {
			...
			for (int ch = 0; ch < O; ++ch) {
				aro[ch] += weight * o[ch]; // TODO: check
				const float &dL_dchannel_obj = dL_dpixel_obj[ch];
				Register_dL_dobjects[ch] += weight * dL_dchannel_obj;
				dL_dalpha += ((o[ch] * T) - (1.0f / (1.0f - alpha)) * (-aro[ch])) * dL_dchannel_obj;
			}
...
void BACKWARD::render(
	...
	const float* sampled_T, const float* sampled_ar, const float* sampled_aro, const float* sampled_ard,
	...
	float* dL_dinvdepths)
{
	...
	PerGaussianRenderCUDA<NUM_CHANNELS_3DGS, NUM_OBJECTS> <<<((B*32) + THREADS - 1) / THREADS,THREADS>>>(
		ranges,
		point_list,
		W, H, B,
		per_bucket_tile_offset,
		bucket_to_tile,
		sampled_T, sampled_ar, sampled_aro, sampled_ard,
		bg_color,
		means2D,
		conic_opacity,
		colors,
