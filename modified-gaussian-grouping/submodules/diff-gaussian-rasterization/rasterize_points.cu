	...
	const bool debug)
{
  ...
  if(P != 0)
  {
	  ...
	  auto tup = CudaRasterizer::Rasterizer::forward(
		...
		out_objects.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(rendered, num_buckets, out_color, out_objects, out_invdepth, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer);
}
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  dL_dout_objects.contiguous().data_ptr<float>(),
	  ...
	  dL_dobjects.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dobjects, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_ddc, dL_dsh, dL_dscales, dL_drotations);
}
...
void applyWeightsGaussiansCUDA(
    ...
    const torch::Tensor& campos,
	const bool prefiltered,
    const torch::Tensor& image_weights,
	const torch::Tensor& cnt,
	const bool debug)
{
  if (means3D.ndimension() != 2 || means3D.size(1) != 3)
  {
