...
def resnet101(pretrained=True, extra_dim=0):
    model = ResNet(Bottleneck, [3, 4, 23, 3], extra_dim)
    if pretrained:
        load_weights_add_extra_dim(model, model_zoo.load_url(model_urls['resnet101']), extra_dim)
    return model

def resnet152(pretrained=True, extra_dim=0):
    model = ResNet(Bottleneck, [3, 8, 36, 3], extra_dim)
    if pretrained:
        load_weights_add_extra_dim(model, model_zoo.load_url(model_urls['resnet152']), extra_dim)
    return model
