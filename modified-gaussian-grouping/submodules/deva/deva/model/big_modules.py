...
class PixelEncoder(nn.Module):
    def __init__(self, pix_feat_dim: int):
        ...
        self.maxpool = network.maxpool  # 1/4, 64
    ...
    def forward(self, x) -> Tuple[Tuple[torch.Tensor, torch.Tensor, torch.Tensor], torch.Tensor]:
        x = self.conv1(x)
        x = self.bn1(x)
...
class MaskEncoder(nn.Module):
    ...
    def forward(self,
                ...
                *,
                ...
                chunk_size: int = -1) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(0, num_objects, chunk_size):
            ...
            g_chunk = self.relu(g_chunk)  # 1/2, 64
