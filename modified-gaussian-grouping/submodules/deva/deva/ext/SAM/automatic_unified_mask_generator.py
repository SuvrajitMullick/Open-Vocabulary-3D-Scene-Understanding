...
def filter_masks(keep: torch.Tensor, masks_result) -> List[Dict[str, Any]]:
...
class UnifiedSamAutomaticMaskGenerator:
    def __init__(
        self,
        model: Sam,
        ...
        Arguments:
          ...
          unifier: object with a unify() method. When provided, 
            generate() returns a single unified mask list.
          nms_iou_thr (float): IoU threshold passed to masks_update().
          nms_score_thr (float): Score threshold passed to masks_update().
          nms_inner_thr (float): Inner-containment threshold passed to
            masks_update().
        """
        ...
        self.nms_inner_thr = nms_inner_thr
    ...
    def generate(self,
        ...
        Returns:
           list(dict(str, any)): A single list of unified mask records. Each record is
        ...
        curr_anns_s, curr_anns_m, curr_anns_l = masks_update(
            curr_anns_s, curr_anns_m, curr_anns_l, iou_thr=self.nms_iou_thr, 
            score_thr=self.nms_score_thr, inner_thr=self.nms_inner_thr)

        masks_unified = self.unifier.unify(curr_anns_l, curr_anns_m, curr_anns_s, image)
        masks_np = np.stack([m['segmentation'] for m in masks_unified], axis=0)
        ious_np = np.array([m['predicted_iou'] for m in masks_unified], dtype=np.float32)
        
        final_mask_data = MaskData()
        final_mask_data["masks"] = torch.from_numpy(masks_np).to(device=self.predictor.device)
        final_mask_data["iou_preds"] = torch.from_numpy(ious_np).to(device=self.predictor.device)
    ...
    def _process_crop(
        ...
        for (points,) in batch_iterator(self.points_per_batch, points_for_image):
            batch_data, batch_data_s, batch_data_m, batch_data_l = self._process_batch(points, negative_points, cropped_im_size, crop_box, orig_size)
            data.cat(batch_data)
